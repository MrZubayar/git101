...
% Ensure 'original_signal' is a column vector
if size(original_signal, 1) == 1
    original_signal = original_signal';
end

% Reshape 'original_signal' to [P.noOfSamples, 1, P.nRand]
original_signal_expanded = reshape(original_signal, [P.noOfSamples, 1, P.nRand]);

% Compute the difference between filtered signals and noisy signals
difference_noisy = filtered_signal - original_signal_expanded;

% Compute the metric based on the difference
metric_values_noisy = compute_difference_metric(difference_noisy);

% Plot the metric across M values
plot_difference_metric_noisy(metric_values_noisy, P.Lee.M_values);
