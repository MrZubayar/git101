...
function generate_plots_for_filter(filter_name, filtered_signal, signal, P, original_signal, selected_signal_type, noise_level, noise_level_vector, quality_metrics)
    ...
    % Initialize a struct to hold all metrics data
    all_metrics_data = struct();
    ...
    for mm = 1:length(quality_metrics)
        ...
        % Store the metric values in the struct
        all_metrics_data.(selected_metric) = struct('M_values', P.Lee.M_values, 'metric', metric_values);
    ...
    % After calculating all metrics, call the function to write them to a .tex file
    write_metrics_to_tex(all_metrics_data, filter_name, selected_signal_type, noise_level_vector);
