function metric_value = quality_evaluation_metrics(selected_metric, filtered_signal, original_signal, P)
    ...
    % Calculate the average metric over all realizations for each M value
    metric_value = mean(metric_value, 2);  % Compute the mean across the second dimension (realizations)
    ...
    for mm = 1:length(P.Lee.M_values)
        fprintf('M = %d: %.2f\n', P.Lee.M_values(mm), metric_value(mm));
    end
    
    % Overall average across all M values (optional, for reference)
    overall_average = mean(metric_value);
