function plot_and_save_figures(filter_name, signal, original_signal, mean_filtered_signal, ...
                               ...
                               snr_values, selected_signal_type, noise_level)
    ...
    for rr = 1:P.nRand
        plot(P.Lee.M_values, snr_values(:, rr), 'Color', [0.8, 0.8, 0.8]);
    end

    plot(P.Lee.M_values, mean(snr_values, 2), '-x', 'LineWidth', 1, 'Color', 'r');

    title(strrep(sprintf('Mean SNR (Noise: %.2f) across Realizations for Different M Values \n (%s) using %s Filter', noise_level, selected_signal_type, filter_name),'_','\_'));
    xlabel('M Value');
    ylabel('SNR (dB)');
